...
export async function processVideoDownloadJob(
  ...
  const downloadStore = config.mediaStores.find(
    (ms) => ms.type == "local" && ms.id == config.download?.targetStoreId
  );

  if (!downloadStore || !downloadStore.path) {
    throw new Error(
      `Download folder for store ${config.download?.targetStoreId} not found.`
    );
  }

  let generatedThumb: ThumbnailContent | undefined = undefined;
  if (download.videoPath && !download.thumbnailPath) {
    generatedThumb = await extractThumbnails(download.videoPath, 1, "webp");
  ...
  const targetFolder = path.join(
    ...
    `${
      download.infoData.channel_id ||
      download.infoData.uploader_id ||
      download.infoData.uploader
    }`,
    `${videoId}`
  );
  await mkdir(targetFolder, { recursive: true });

  const targetVideoPath = download.videoPath && path.join(targetFolder, `${videoId}${path.extname(download.videoPath)}`);
  ...
  if (download.infoPath) {
    await move(
      download.infoPath,
      path.join(targetFolder, `${videoId}.info.json`)
    );
  }
  ...
  if (download.thumbnailPath) {
    await move(
      ...
      path.join(
        targetFolder,
        `${videoId}${path.extname(download.thumbnailPath)}`
      )
    );
  }
